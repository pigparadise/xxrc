...
// 接收聊天信息
func (u *User) OnChatKickoff(resp *xxc.Response) {
	log.Printf("OnChatKickoff, user<%s> :<%s,%s>", u.config.User, resp.Result, resp.Message)
	__del_user(g_user_mgr, u.config.Host, u.config.User)
}
...
func (u *User) SayToGroup(gid string, content string) error {
	log.Printf("SayToGroup, gid<%s>, content<%s>", gid, content)
	/*
...
func (u *User) SayToUserById(user_id int, content string) error {
	log.Printf("SayToUserById, %s, %s", user_id, content)
	group := u.QueryOne2OneGroup(user_id)
	if group != nil {
	...
	// 走到这里，说明这两个人之前没有私聊过
	// 先创建一个 one2one Group
	gid, err := u.CreateOne2OneGroup(user_id)
	if err != nil {
		return err
	}
	return u.say(gid, content)
...
func CreateUser(config *xxc.ClientConfig) (*User, error) {
